...
> Sistema de orquestração multi-agente para Codex
...
O arquivo `.mcp.json` e a configuracao oficial dos servidores MCP deste repositorio. O Astra deve usa-los sob demanda, conforme a responsabilidade da tarefa, sem executar comandos destrutivos ou instalar dependencias silenciosamente.

### Ambiente Codex ativo

- Config Codex do projeto: `.codex/config.toml`.
- Skill repo obrigatoria: `.agents/skills/oraculo-ambiente/SKILL.md`.
...
- Validador MCP: `scripts/mcp/validar_mcp.py`.
- Comando de validacao: `.ferramentas\mcp\python\.venv\Scripts\python.exe scripts\mcp\validar_mcp.py --todos`.
- MCPs ativos e validados devem ficar em `.mcp.json` e `.codex/config.toml`. Nao duplicar configuracao fora desses arquivos.
- MCPs com dependencia nativa quebrada no Windows/Node atual ficam fora do nucleo ativo ate teste verde. Hoje: `codegraph` e `repowise`.

### Ordem de inicializacao

1. Ler `.claude/contexto.md`, `.claude/skill.md` e `.claude/00_orquestrador.md`.
2. Consultar o Git MCP para detectar alteracoes preexistentes antes de editar.
3. Usar filesystem MCP para leitura e edicoes pequenas e rastreaveis.
4. Usar Serena para simbolos, referencias e refatoracoes semanticas.
5. Usar codegraph ou Repowise para dependencias, callers, codigo morto e decisoes.
6. Usar Semgrep antes de concluir mudancas em autenticacao, secrets, API, persistencia ou execucao.
7. Usar Figma MCP somente em tarefas de frontend.
8. Executar testes focados imediatamente apos cada edicao significativa.

### Uso dos servidores

| Servidor | Uso obrigatorio |
|---|---|
| `filesystem` | Ler e editar arquivos dentro deste workspace |
| `git` | Diff, historico, blame, estado preexistente e rastreabilidade |
| `semgrep` | Seguranca, secrets, falhas de API e padroes perigosos |
| `codegraph` | Grafo de imports, callers e impacto de mudancas |
| `serena` | Simbolos, referencias e refatoracoes semanticas |
| `repowise` | Codigo morto, decisoes, sinais Git e visao arquitetural |
| `figma` | Apenas quando a tarefa exigir design ou frontend |

### Guardioes e skills

Antes de alterar calculo, risco, sinal, ordem, persistencia financeira, autotrader ou API que possa liberar operacao, o Astra deve consultar:

- `.claude/02_guardiao.md`: veto e seguranca financeira;
- `.claude/04_quant.md`: EV, taxas, calibracao, IC, Brier e edge;
- `.claude/05_execucao.md`: ordens, fills, idempotencia, breaker e shutdown;
- `.claude/06_persistencia.md`: SQLite, transacoes, locks e Unit of Work;
- `.claude/07_testes.md`: caracterizacao, regressao e cobertura;
- `.claude/01_revisor.md`: atualizar `.claude/contexto.md` depois da mudanca.

Para sinais e features, consultar `.claude/03_sinais.md`. Para decomposicao, `.claude/08_refatoracao.md`. Para logs e metricas, `.claude/09_observabilidade.md`.

Nenhum MCP, IA ou agente pode liberar conta real, remover gate financeiro, resetar halt ou alterar limite de risco sem evidencia, teste e aprovacao explicita do fluxo de guardiao.

### Politica de repositorio limpo

O Astra deve manter o repositorio limpo, mas nunca deve apagar ou mover arquivo apenas pelo nome, por ausencia de referencia textual ou por estar fora da estrutura-alvo.

Antes de mover qualquer arquivo para `lixo-eletronico/`, deve:

1. verificar imports, referencias semanticas, scripts, entrypoints, testes, CI, configuracoes e documentacao;
2. consultar Git, codegraph ou Repowise quando disponivel;
3. confirmar que nao e runtime, migracao, fixture, dado necessario ou compatibilidade publica;
4. registrar origem, data, razao e evidencia em `lixo-eletronico/README.md`;
5. mover em vez de apagar, preservando o caminho original no registro;
6. executar os testes e verificacoes relevantes;
7. pedir aprovacao humana antes de remover permanentemente a quarentena.

Nunca mover automaticamente `.env`, chaves, bancos ativos, backups, logs de producao, dados de usuario, migracoes ou artefatos usados pelo CI. `lixo-eletronico/` e uma quarentena reversivel, nao um deposito irreversivel.
...
```
> Os arquivos de agente ficam na RAIZ de `.Codex/`, não numa subpasta `agentes/`.
...
> Consulte `.Codex/contexto.md` para o estado atual de cada fase.
