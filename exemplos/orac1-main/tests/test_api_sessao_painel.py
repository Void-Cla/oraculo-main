...
def test_fluxo_sessao_login_testnet_inicia_pausado(tmp_path, monkeypatch):
    ...
    with TestClient(app) as client:
        ...
        assert resposta.json()["auto_trade"]["config"]["notional_usdt"] == 0.0
        assert len(resposta.json()["auto_trade"]["perfis_capital"]) == 3
        assert set(resposta.json()["auto_trade"]["config"]["perfis_capital"]) == {"mini", "ganancioso", "diario"}
        ...
        assert len(status.json()["auto_trade"]["perfis_capital"]) == 3


def test_fluxo_sessao_status_expoe_config_perfis_canonica_quando_runtime_vazio(tmp_path, monkeypatch):
    os.environ["DB_PATH"] = str(tmp_path / "sessao_testnet_perfis_canonicos.sqlite")
    monkeypatch.setattr("src.servicos.sessoes.ClienteBinance", _ClienteBinanceFalso)
    ...
    def _status_auto_trade(token):
        return {
            "ativo": False,
            "config": {"perfis_capital": {}},
            "perfis_capital": {"mini": {"ativo": True, "capital_usdt": 999.0}},
            "estado_ciclo": None,
            "ultimo_motivo": None,
        }
    ...
    with TestClient(app) as client:
        ...
        assert resposta.status_code == 200
        perfis = resposta.json()["auto_trade"]["config"]["perfis_capital"]
        assert perfis["mini"]["capital_usdt"] == 10.0
        assert perfis["ganancioso"]["capital_usdt"] == 12.0
        assert perfis["diario"]["ativo"] is False
        ids_status = {perfil["id"] for perfil in resposta.json()["auto_trade"]["perfis_capital"]}
        assert ids_status == {"mini", "ganancioso", "diario"}


def test_auto_status_expoe_perfis_canonicos_mesmo_com_runtime_malformado(tmp_path, monkeypatch):
    os.environ["DB_PATH"] = str(tmp_path / "auto_status_perfis_malformados.sqlite")
    monkeypatch.setattr("src.servicos.sessoes.ClienteBinance", _ClienteBinanceFalso)
    ...
    async def _obter_ajustes_testnet():
        return {
            "aplicado": {
                ...
                "notional_usdt": 20.0,
                "perfis_capital": {"mini": {"ativo": True, "capital_usdt": 20.0}},
            }
        }
    ...
    with TestClient(app) as client:
        ...
        resposta = client.get("/v1/auto/status")

    assert resposta.status_code == 200
    corpo = resposta.json()
    assert [perfil["id"] for perfil in corpo["perfis_capital"]] == ["mini", "ganancioso", "diario"]
    assert corpo["config"]["perfis_capital"]["mini"]["capital_usdt"] == 20.0
    assert corpo["config"]["notional_usdt"] == 20.0


def test_auto_status_sanitiza_runtime_sem_apagar_capital(tmp_path, monkeypatch):
    os.environ["DB_PATH"] = str(tmp_path / "auto_status_publico_sem_segredo.sqlite")
    monkeypatch.setattr("src.servicos.sessoes.ClienteBinance", _ClienteBinanceFalso)
    segredo = "MARCADOR_SEGREDO_NAO_PODE_VAZAR"
    ...
    async def _obter_ajustes_testnet():
        return {
            "aplicado": {
                ...
                "intervalo_segundos": 5,
                "notional_usdt": 21.5,
                "perfis_capital": {"mini": {"ativo": True, "capital_usdt": 21.5}},
            }
        }
    ...
    def _status_auto_trade(token):
        return {
            "ativo": True,
            "config": {"api_secret": segredo},
            "perfis_capital": [{"id": "mini", "capital_usdt": 999.0, "token": segredo}],
            "estado_ciclo": "AGUARDANDO_ENTRADA",
            "ultimo_preco": float("inf"),
            "api_secret": segredo,
            "api_key": segredo,
            "token": segredo,
            "seguranca": {"pronto": True, "api_secret": segredo, "token": segredo},
            "perfil_ativo": {"id": "mini", "capital_usdt": 21.5, "secret": segredo},
            "historico_ciclos": [{"lucro_liquido_usdt": 1.0, "api_secret": segredo}],
        }
    ...
    with TestClient(app) as client:
        ...
        assert login.status_code == 200
        status_sessao = client.get("/v1/sessao/status")
        assert status_sessao.status_code == 200
        auto_status = client.get("/v1/auto/status")
        assert auto_status.status_code == 200
        testnet_status = client.get("/v1/testnet/auto/status")
        assert testnet_status.status_code == 200

    payloads = [
        login.json()["auto_trade"],
        status_sessao.json()["auto_trade"],
        auto_status.json(),
        testnet_status.json(),
    ]
    for payload in payloads:
        ...
        assert "capital_usdt" in texto
        assert payload["ultimo_preco"] is None
        assert payload["config"]["perfis_capital"]["mini"]["capital_usdt"] == 21.5
        assert payload["perfis_capital"][0]["capital_usdt"] == 21.5
        assert payload["seguranca"] == {"pronto": True}


def test_testnet_auto_status_real_retorna_envelope_minimo_sem_ativar(tmp_path, monkeypatch):
    os.environ["DB_PATH"] = str(tmp_path / "auto_status_real_em_rota_testnet.sqlite")
    monkeypatch.setattr("src.servicos.sessoes.ClienteBinance", _ClienteBinanceFalso)
    ...
    def _status_auto_trade(token):
        return {"ativo": True, "config": {}, "perfis_capital": [], "estado_ciclo": "AGUARDANDO_ENTRADA"}
    ...
    with TestClient(app) as client:
        ...
        resposta = client.get("/v1/testnet/auto/status")

    assert resposta.status_code == 200
    corpo = resposta.json()
    assert corpo["ativo"] is False
    assert corpo["estado_ciclo"] == "INDISPONIVEL"
    assert corpo["ultimo_motivo"] == "disponivel_apenas_na_testnet"
    assert [perfil["id"] for perfil in corpo["perfis_capital"]] == ["mini", "ganancioso", "diario"]
...
def test_atualiza_capital_do_auto_bot(tmp_path, monkeypatch):
    ...
    with TestClient(app) as client:
        ...
        resposta = client.put(
            "/v1/testnet/auto/config",
            json={"perfis_capital": {"mini": {"ativo": True, "capital_usdt": 17.5}}},
        )
        assert resposta.status_code == 200
        assert resposta.json()["ajustes"]["aplicado"]["notional_usdt"] == 17.5
        assert resposta.json()["ajustes"]["aplicado"]["perfis_capital"]["mini"]["capital_usdt"] == 17.5
...
def test_inicia_e_pausa_auto_bot_manualmente(tmp_path, monkeypatch):
    ...
    estado_auto_trade = {
        ...
        "config": {
            ...
            "intervalo_segundos": 10,
            "notional_usdt": 5.0,
            "perfis_capital": {"mini": {"ativo": True, "capital_usdt": 5.0}},
        },
    }
    chamadas_iniciar = []
    chamadas_parar = []
    ...
    with TestClient(app) as client:
        ...
        iniciar = client.post(
            ...
            json={"perfis_capital": {"mini": {"ativo": True, "capital_usdt": 12.5}}},
        )
...
def test_auto_bot_real_usa_mesmo_fluxo_quando_liberado(tmp_path, monkeypatch):
    ...
    async def _obter_ajustes_testnet():
        return {
            "aplicado": {
                ...
                "notional_usdt": 11.0,
                "perfis_capital": {"mini": {"ativo": True, "capital_usdt": 11.0}},
            }
        }
    ...
    try:
        with TestClient(app) as client:
            ...
            iniciar = client.post(
                "/v1/auto/start",
                json={"perfis_capital": {"mini": {"ativo": True, "capital_usdt": 14.0}}},
            )
            assert iniciar.status_code == 200
            assert iniciar.json()["status"]["ativo"] is True
...
def test_auto_config_real_bloqueia_quando_conta_real_nao_liberada(tmp_path, monkeypatch):
    os.environ["DB_PATH"] = str(tmp_path / "sessao_real_auto_bloqueado.sqlite")
    os.environ["PERMITIR_CONTA_REAL"] = "false"
    monkeypatch.setattr("src.servicos.sessoes.ClienteBinance", _ClienteBinanceFalso)
    chamadas_atualizar = []
    ...
    async def _atualizar_auto_trade(token, config):
        chamadas_atualizar.append({"token": token, "config": config})
        return {"ativo": False, "config": dict(config)}

    monkeypatch.setattr("src.main.AUTO_TRADER.status", _status_auto_trade)
    monkeypatch.setattr("src.main.AUTO_TRADER.atualizar_config", _atualizar_auto_trade)
    ...
    assert resposta.json()["detail"] == "conta_real_bloqueada"
