...
def test_risk_engine_hold_retorna_motivo_unico_sem_ruido_de_saldo():
    sinal = _sinal_base()
    sinal["acao"] = "HOLD"
    aprovacao = avaliar_sinal_para_usuario(
        usuario=_usuario(),
        sinal=sinal,
        saldo={"saldo_total": 0.0, "saldo_livre": 0.0},
        estado_execucao={},
    )

    assert aprovacao["aprovado"] is False
    assert aprovacao["motivos"] == ["sinal_hold"]
    assert aprovacao["notional_sugerido"] == 0.0
    assert aprovacao["fracao_capital"] == 0.0


def test_risk_engine_nao_duplica_fracao_invalida_quando_saldo_insuficiente():
    aprovacao = avaliar_sinal_para_usuario(
        usuario=_usuario(),
        sinal=_sinal_base(),
        saldo={"saldo_total": 0.0, "saldo_livre": 0.0},
    ...
    assert aprovacao["aprovado"] is False
    assert "saldo_insuficiente" in aprovacao["motivos"]
    assert "fracao_calculada_invalida" not in aprovacao["motivos"]


def test_risk_engine_rejeita_acao_invalida():
    sinal = _sinal_base()
    sinal["acao"] = "CLOSE"
    ...
    assert aprovacao["aprovado"] is False
    assert aprovacao["acao"] == "HOLD"
    assert aprovacao["motivos"] == ["acao_invalida"]
    assert aprovacao["notional_sugerido"] == 0.0
    assert aprovacao["fracao_capital"] == 0.0


def test_risk_engine_testnet_nao_aprova_lucro_negativo(monkeypatch):
    monkeypatch.delenv("LUCRO_MINIMO_TESTNET", raising=False)
    monkeypatch.delenv("LUCRO_MINIMO_USDT_TESTNET", raising=False)
    usuario = _usuario()
    usuario["testnet"] = True
    usuario["risk_config"]["lucro_liquido_minimo"] = -1.0
    usuario["risk_config"]["lucro_liquido_minimo_usdt"] = -1.0
    sinal = _sinal_base()
    sinal["lucro_liquido_esperado_pct"] = -0.001
    ...
    assert "lucro_liquido_usdt_abaixo_do_minimo" in aprovacao["motivos"]
    assert aprovacao["lucro_liquido_esperado_usdt"] == 0.0
